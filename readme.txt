[center][url="https://smftricks.com/"][img]https://smftricks.com/logos/logo.png[/img][/url]
Free & Premium Themes for SMF
[/center]

[b][size=5]NameX[/size][/b]
[size=2]Developed by: [url="https://smftricks.com/index.php?action=profile;u=1"]Diego Andrés[/url][/size]
[size=2]Designed by: [url="https://smftricks.com/index.php?action=profile;u=17"]Harold Zapata[/url][/size]

[url="https://github.com/SMFTricks/NameX"][img]https://img.shields.io/github/v/release/SMFTricks/NameX.svg?style=flat&logo=github&color=green[/img][/url] [img]https://img.shields.io/badge/License-MIT-248049[/img] [img]https://img.shields.io/badge/SMF-2.1-3f73a0[/img] [img]https://img.shields.io/badge/Color%20Variants-Yes-6041a3[/img] [img]https://img.shields.io/badge/Dark%20Mode-Yes-4d827f[/img] [img]https://img.shields.io/badge/RLT%20Support-Yes-bf9d73[/img]

[float=right max=40%]

Preview:
[img width=350]https://raw.githubusercontent.com/SMFTricks/NameX/main/_assets/preview.gif[/img]
[url=http://demo21.smftricks.com/index.php?theme=4]Demo Online[/url]
[/float]
[color=#2346CD][size=4]☑️ Features[/size][/color]
[list]
[li][font=Sans-serif]FontAwesome[/font][/li]
[li][font=Sans-serif]Social Networks[/font][/li]
[li][font=Sans-serif]Logo URL[/font][/li]
[li][font=Sans-serif]Forum Width[/font][/li]
[li][font=Sans-serif]Separate sticky topics[/font][/li]
[li][font=Sans-serif]Avatars
[list type=circle]
[li][font=Sans-serif]Boards[/font][/li]
[li][font=Sans-serif]Topics[/font][/li]
[li][font=Sans-serif]Online Users[/font][/li]
[li][font=Sans-serif]Recent Posts[/font][/li]
[li][font=Sans-serif]Memberlist[/font][/li]
[/list]
[/li]
[li][font=Sans-serif]Menu Customization
[list type=circle]
[li][font=Sans-serif]Disable Menu Icons[/font][/li]
[li][font=Sans-serif]Add Log out, Log in and Sign up buttons[/font][/li]
[li][font=Sans-serif]Remove Specific Menu [/font][/li]
[li][font=Sans-serif]Community Button for Grouping[/font][/li]
[/list]
[/li]
[li][font=Sans-serif]Select CDN for Fonts[/font][/li]
[/list]

[color=#bf307a][size=4]📦 Installation[/size][/color]
It is installed as a traditional SMF Theme for SMF 2.1.x, just grab the latest release of the package and go to: [i]Admin > Configuration > Themes and Layout > Install a New Theme > From an Archive > Select File > Upload[/i].

You can find detailed instructions and/or more documentation about the process in [url=https://wiki.simplemachines.org/smf/Installation_-_How_do_I_install_a_theme]Installation - How do I install a theme[/url].

[color=#315CFB][size=4]🌐 Localization[/size][/color]
Translations are all based off the [i][url=https://github.com/SMFTricks/Theme-Customs]Theme Customs[/url][/i] strings, and you can contribute directly on [b][url=https://crowdin.com/project/smf-theme-customs]Crowdin[/url][/b].

[color=#cc5d31][size=4]🔨 Changelog[/size][/color]
Review the changes and patches from the different releases/versions of the package in the [url=https://github.com/SMFTricks/NameX/blob/main/CHANGELOG.md]Changelog[/url]